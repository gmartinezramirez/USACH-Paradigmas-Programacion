...
(define muestra-gonzalo
  (muestra "18045598-1" 10 28 "M" "Sotero del rio" #t #t "enf pre1" "enf detect1"))

(define muestra-persona1
  (muestra "19045598-1" 20 29 "M" "Sotero del rio" #t #f "enf pre2" "enf detect2"))
...
(define set-muestra-evaluada-enf-detect
  (lambda (lista-muestras rut id evaluada? enf-detect)
    (muestra rut id
             ...
             (get-enf-pre (car lista-muestras))
             enf-detect)))
...
(define muestra1-gonzalo
  (muestra "18045598-1" 10 28 "M" "Sotero del rio" #t #t "catarro" "gripe"))

(define muestra2-gonzalo
  (muestra "18045598-1" 12 28 "M" "Sotero del rio" #f #t "gripe" "toz"))

(define muestra1-otra-persona
  (muestra "20111528-k" 13 29 "M" "Sotero del rio" #t #t "toz" "nauseas"))

(define listado-de-ejemplo-rut
  (lista-muestras muestra1-gonzalo muestra2-gonzalo muestra1-otra-persona))

(display "Modificar muestra - ejemplos \n")
(define nueva-lista-cambiada
  (modificar-muestra listado-de-ejemplo-rut "18045598-1" 10 #t "flojera"))

nueva-lista-cambiada
...
(define edad-entre-min-y-max?
  (lambda (min max)
    (lambda (muestra)
      (cond
        [(and (<= min (get-edad muestra))
              (>= max (get-edad muestra)))
         #t]
        [else #f]))))
...
(define es-misma-enf-detectada?
  (lambda (enf-detectada)
    (lambda (muestra)
      (cond
        [(eq? enf-detectada (get-enf-detect muestra))
         #t]
        [else #f]))))
